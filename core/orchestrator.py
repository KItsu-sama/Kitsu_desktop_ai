            ...
            if memories:
                ...
                for memory in memories:
                    ...
                    if total_size + len(memory_text) <= max_size:
                        ...
                        # Truncate safely without splitting UTF-8 characters
                        remaining = max_size - total_size
                        if remaining > 50:  # Only add if meaningful content remains
                            # Encode and truncate, then decode (handles UTF-8 safely)
                            try:
                                truncated = memory_text.encode('utf-8')[:remaining].decode('utf-8', errors='ignore')
                                memory_texts.append(truncated + "...")
                            except Exception:
                                # Fallback: character-based truncation if encode fails
                                memory_texts.append(memory_text[:remaining] + "...")
                        break
                
                return " ".join(memory_texts)
        except Exception as e:
            logger.debug("Failed to retrieve memory context: %s", e)
        return None
