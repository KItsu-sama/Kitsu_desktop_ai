...
from utils.logger import set_debug_output, is_debug_output_enabled
...
class CommandRouter:
    ...
    async def _cmd_mood(self, parts, full_cmd):
        if len(parts) < 2:
            return {"success": False, "output": "❌ Usage: /mood <behave|mean|flirty>"}
        mood = parts[1].lower()
        emotion_engine = self.controller.emotion_engine
        ...
        if mood == "clear":
            if hasattr(emotion_engine, 'clear_mood_override'):
                emotion_engine.clear_mood_override()
                return {"success": True, "output": "✨ Manual mood override cleared"}
        elif mood in ("behave", "mean", "flirty"):
            persist = "--persist" in parts or "persist" in parts
            if hasattr(emotion_engine, 'set_mood'):
                emotion_engine.set_mood(mood, persist=persist)
                return {"success": True, "output": f"✨ Mood set to: {mood}"}
        return {"success": False, "output": "❌ Invalid mood. Use: behave, mean, or flirty"}

    async def _cmd_style(self, parts, full_cmd):
        if len(parts) < 2:
            from personality.emotion_config import VALID_STYLES
            return {"success": False, "output": f"❌ Usage: /style <{', '.join(sorted(VALID_STYLES))}>"}
        style = parts[1].lower()
        from personality.emotion_config import validate_style
        emotion_engine = self.controller.emotion_engine
        if not emotion_engine:
            return {"success": False, "output": "❌ Emotion engine not available"}
        
        if validate_style(style) and hasattr(emotion_engine, 'set_style'):
            emotion_engine.set_style(style)
    ...
    async def _trigger_micro_train_now(self, ts) -> str:
        try:
            from core.learning.micro_trainer import MicroTrainer, MicroTrainRequest, MicroTrainConfig
        except Exception as e:
