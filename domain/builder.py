...
class PersonalityBuilder:
    ...
    def _build_from_emotions(self, emotion_weights: List[EmotionWeight], 
        ...
        Uses normalized weighted averaging instead of sequential blending.
        ...
        mood_result = self._weighted_average(mood_vectors, mood_weights) if mood_vectors else None
        style_result = self._weighted_average(style_vectors, style_weights) if style_vectors else None
        state_result = self._weighted_average(state_vectors, state_weights) if state_vectors else None
        ...
        if style_result:
            ...
            weights.append(0.35)
        if state_result:
            components.append(state_result)
            weights.append(0.20)

        components.append(role_vector)
        weights.append(0.05)
