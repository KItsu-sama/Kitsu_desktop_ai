...
# The context type here is intentionally treated as dynamic because
# RequestContext may expose simhash as a loose attribute.
...
def compute_simhash(text: str) -> str:
    ...
    Uses MD5 for stable hashing across sessions (not Python's randomized hash()).
    """
    ...
    # Use MD5 instead of randomized hash() for stable cache keys across sessions
...
async def on_input_received(ctx: RequestContext):
    ...
    Budget: 10ms.
    """
    # Preprocess is an intermediate stage and should not mark the request as responded.
    # Only handlers that emit RESPONSE_READY should call can_respond(ctx).

    # RequestContext may not declare simhash in its type hints.
    # Compute and attach dynamically.
    setattr(ctx, "simhash", compute_simhash(ctx.text))
...
register("INPUT_RECEIVED", on_input_received)
