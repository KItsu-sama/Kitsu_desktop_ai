...
from .terminal_ui import display_splash as _display_splash
...
class Launcher:
    ...
    async def start(self):
        ...
        try:
            ...
            self.state_store.set_state(RuntimeState.BOOTING, reason="Application startup initiated")
            ...
            try:
                ...
                _display_splash(mode="text", model=model, use_rich=True)
            except Exception as e:
                logger.warning(f"Splash screen failed: {e}")
                # No global LOGO variable in this module; fall back to splash-less mode.
                # (If splash fails, we still want the runtime error UI hooks enabled.)


            # Ensure global error hooks are installed early
            try:
                from .runtime_error_hooks import install_runtime_error_hooks

                install_runtime_error_hooks()
            except Exception:
                logger.exception("Failed to install runtime error hooks")
            ...
            if not check_setup_complete():
                ...
                try:
                    ...
                    # In gateway / ephemeral environments, avoid interactive wizard output.
                    interactive = not bool(os.environ.get("KITSU_GATEWAY_ONLY"))
                    init_success = await run_first_run(interactive=interactive)
                    if not init_success:
                        msg = "First-run setup failed"
                        print(f"\n❌ {msg}")
                        self.state_store.set_state(RuntimeState.FAILED, reason=msg)
                        return False
                except Exception as e:
