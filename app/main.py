...
5. KisuLauncher.startup() → modules + engine
...
from app.kisu_launcher import KisuLauncher  # or Launcher from previous merge
...
async def start_engine(container: Any) -> int:
    ...
    launcher = KisuLauncher(container)
    ...
    try:
        ...
        if not result and container.profile.tier != 'micro':  # safe mode check
            logger.warning('Primary startup failed → falling back to ultra-low safe mode')
        ...
        if result:
            ...
            logger.error("❌ Final startup failure")
            return 1
