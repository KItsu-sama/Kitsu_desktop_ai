...
from config.schema import load_profile_config, load_and_validate
...
def select_profile(profile_override: str | None = None, safe_mode: bool = False) -> HardwareProfile:
    ...
    3. KISU_PROFILE env var
    4. KISU_SAFE_MODE env var
    5. Advanced hardware detection
    6. Simple hardware detection fallback
    """
    if safe_mode or os.environ.get('KISU_SAFE_MODE', '').lower() in ('1', 'true', 'yes'):
    ...
    elif os.environ.get('KISU_PROFILE'):
        profile_path = _resolve_profile_override(os.environ['KISU_PROFILE'])
        logger.info('Using KISU_PROFILE env: %s', profile_path.name)
    else:
