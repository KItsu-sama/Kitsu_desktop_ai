...
from runtime.communication.events import (
    # Legacy events
...
from runtime.legacy.application import Application
from domain.interaction.input_manager import InputManager
from domain.interaction.input_mux import InputMultiplexer, get_input_mux
from runtime.systems.system_monitor import SystemMonitor
...
from runtime.core.module_registry import get_module_registry, ModuleType
try:
...
class Orchestrator(ModuleContract):
    ...
    def __init__(self, runtime_config: Optional[Any] = None):
        ...
        from runtime.communication.events import EventBus
        event_bus = EventBus()  # Use legacy bus for compatibility
        self.application = Application(self, event_bus)
        self.input_manager = None  # Will be initialized after emotion_manager and core_memory
        self.input_mux = None  # Will be initialized after event_bus is ready
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            # Initialize InputManager now that emotion_manager and core_memory are ready
            self.input_manager = InputManager(
                message_bus=bus,  # Use the communication bus
                emotion_manager=self.emotion_manager,
                core_memory=self.core_memory,
                llm_fallback=getattr(self, 'llm_fallback', None),
                slm=getattr(self, 'slm', None),
                judge=None,  # Will be added when Judge module is integrated
                fast_brain=getattr(self, 'fast_brain', None),
                core_config=self.core_config
            )
            
            # Initialize InputMux for input normalization
            self.input_mux = await get_input_mux(bus)
    ...
    async def _chat_loop(self) -> None:
        ...
        while not (self._shutdown_event and self._shutdown_event.is_set()):
            try:
                # NON-BLOCKING INPUT with timeout to allow shutdown
                try:
                    user_input = await asyncio.wait_for(
                        ...
                        timeout=0.5  # Check shutdown every 0.5 seconds
                    )
                except asyncio.TimeoutError:
...
Any other text will be processed as chat input.
    ...
    def request_stop(self) -> None:
        ...
        # Stop emotion engine and manager background loops
        if self.emotion_engine and hasattr(self.emotion_engine, 'stop'):
            self.emotion_engine.stop()
        if self.emotion_manager and hasattr(self.emotion_manager, 'stop'):
            self.emotion_manager.stop()
    ...
    async def process_input(
        ...
        """
        DEPRECATED: Legacy processing logic. 
        Now delegates to the modern InputManager pipeline.
        
        This is the "thin wrapper" pivot - the Legacy Orchestrator is now the "Courier"
        that routes requests to the modern InputManager instead of being the "Brain".
        """
        # Route to modern InputManager if available
        if self.input_manager:
            try:
                # This sends the input into the modern pipeline 
                # (Preprocess -> Router -> Brain Tier -> Judge)
                return await self.input_manager.process_input(user_input)
            except Exception as e:
                logger.warning(f"InputManager processing failed, falling back to legacy: {e}")
                return await self._legacy_process_input_logic(user_input, context, force_generation_mode)
        
        # Fallback to old logic only if InputManager isn't ready
        logger.warning("InputManager not initialized, falling back to legacy logic")
        return await self._legacy_process_input_logic(user_input, context, force_generation_mode)

    async def _legacy_process_input_logic(self, user_input: str, context: Optional[Dict[str, Any]] = None, force_generation_mode: Optional[str] = None) -> Dict[str, Any]:
