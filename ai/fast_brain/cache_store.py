...
Role: It stores the last 5–10 exchanges in a high-speed RAM buffer. If the user repeats a question exactly, Kisu pulls the answer from here in 0ms. This is crucial for the "Hai! -> Hai!" spam loop, where Kisu can instantly recognize and repeat the user's input without hitting the LLM.
"""
...
class ConversationCache:
    ...
    def add_exchange(self, user_input: str, kisu_response: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        """
        Add a conversation exchange to the cache.
        
        Args:
            user_input: User's message
            kisu_response: Kisu's response
            metadata: Additional metadata
        """
        exchange = {
            "user_input": user_input,
            "kisu_response": kisu_response,
            "timestamp": time.time(),
        ...
        self.exact_matches[normalized_input] = kisu_response
...
def add_to_cache(user_input: str, kisu_response: str, metadata: Optional[Dict[str, Any]] = None) -> None:
    """
    ...
    Args:
        ...
        metadata: Additional metadata
    """
    conversation_cache.add_exchange(user_input, kisu_response, metadata)
