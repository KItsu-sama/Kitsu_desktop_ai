...
class Triggers:
    ...
    def build_plan(self, trigger_name: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def fire_trigger(self, trigger_name: str) -> Optional[Dict[str, Any]]:
        ...
        if trigger_name in self._active_triggers:
            return None  # prevent recursion loop
        
        plan = self.build_plan(trigger_name)

        self._active_triggers.add(trigger_name)
        try:
            if not plan:
                return None
            ...
            self._active_triggers.discard(trigger_name)
